...
struct Asset {
  Asset(const char* file_location_in, const char* asset_name_in = NULL,
      ...
      : file_location(file_location_in), name(asset_name_in), guid(guid_in) {}
...
class ResourceBase : public Resource {
  /*------------------------------------------------------------------------------
    ...
    parneting, file metadata and covering any functionality that does not apply
  to all resource types (e.g. rendering) Note
  - logger breaks in the case of multuiple resource managers
  - Need to add functionality for adding, deleting and checking parents and
  children
  ------------------------------------------------------------------------------*/
 public:
  ...
  // Currently returns vao index (opengl) but need to find out directx
  // equivilent
 ...
 protected:
  ...
  Asset asset_;
  // This needs to be a pointer to prevent requirement for default Logger
  // constructor
};
