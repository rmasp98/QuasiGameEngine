...
int main() {
  ...
  // load to graphics is a bit shit. Should be run by rendermanager (figure it out)
  Resource* picture = resource_manager->GetResource("/home/rmaspero/graphics/quasiGameEngine/assets/Dragon_ground_color.jpg", "");
  if (picture != nullptr)
    picture->LoadToGraphics(render_manager);

  Resource* object = resource_manager->GetResource("/home/rmaspero/graphics/quasiGameEngine/assets/Dragon_2.5.3ds");
  if (object != nullptr)
  ...
  do {
    ...
    if (input->IsActionActive(ACTION_UP))
      pos += facing * 0.1f;
    if (input->IsActionActive(ACTION_DOWN))
      pos -= facing * 0.1f;
