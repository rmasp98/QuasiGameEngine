...
GlfwRenderConfig::GlfwRenderConfig(Logger logger, RenderConfigType type)
    : logger_(logger) {
  if (type == RC_DEFAULT) {
...
void GlfwRenderConfig::SetBlend(int src_func, int dest_func, int equation) {
  blend_ = true;
...
void GlfwRenderConfig::ApplyConfig() {
  if (cull_face_)
