...
Resource* ObjectLoader::Load(Asset asset) {
  LOG(TRACE, RESOURCE) << "Loading object from " << asset.file_location;
  ...
  if (scene) {
    for (size_t i_mesh = 0; i_mesh < scene->mNumMeshes; ++i_mesh) {
      ...
      if (!strcmp(mesh->mName.C_Str(), asset.name)) {
        ...
        if (mesh->HasFaces()) {
          LOG(TRACE, RESOURCE) << "Getting faces from '" << asset.name
        ...
        if (mesh->HasPositions()) {
          LOG(TRACE, RESOURCE) << "Getting verticies from '" << asset.name
                               << "', in '" << asset.file_location << "'";
        ...
        if (mesh->HasNormals()) {
          LOG(TRACE, RESOURCE) << "Getting normals from '" << asset.name
                               << "', in '" << asset.file_location << "'";
        ...
        if (mesh->HasTextureCoords(0)) {
          LOG(TRACE, RESOURCE)
              << "Getting texture coordinates from '" << asset.name << "', in '"
              << asset.file_location << "'";

          texture_coords =
              LoadAttribute(mesh->mTextureCoords[0], mesh->mNumVertices,
  ...
  LOG(TRACE, RESOURCE) << "Failed to load " << asset.file_location
...
QgeArray<float> ObjectLoader::LoadAttribute(const aiVector3D* data_in,
  ...
  for (int i_vert = 0; i_vert < num_vertices; ++i_vert) {
    for (int j_comp = 0; j_comp < num_components; ++j_comp) {
...
QgeArray<int> ObjectLoader::LoadFaces(const aiFace* faces_in, int num_faces,
  ...
  for (int i_face = 0; i_face < num_faces; ++i_face) {
    for (int j_index = 0; j_index < num_indices; ++j_index) {
