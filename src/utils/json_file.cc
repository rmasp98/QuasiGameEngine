...
nlohmann::json JsonFile::LoadFile(const char* file_name) {
  if (file_name == nullptr) {
    // LOG(ERROR, UTILS) << "No input configuration file defined";
    // TODO: Set up throwing properly
    throw "";
  }
  // LOG(INFO, UTILS) << "Loading input configuration from '" << file_name <<
  // "'";
  return ParseFile(file_name);
}

nlohmann::json JsonFile::ParseFile(const char* file_name) {
  // LOG(INFO, UTILS) << "Parsing new json schema from '" << file_name << "'";
  try {
