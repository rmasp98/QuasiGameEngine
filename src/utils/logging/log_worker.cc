...
bool LogWorker::Init() {
  // Currently prevents multiple threads from being created could theoretically
  // have mulitple...
  if (!is_init_) {
...
void LogWorker::CollectorLoop() {
  while (is_running_ || !log_queue_.empty()) {
    ...
    if (Pop(log)) {
      ...
      if (log.file_index >= 0 &&
          static_cast<uint>(log.file_index) < file_streams_.size()) {
        // This prevents other thread from altering the vector while we acccess
        // it
        std::lock_guard<std::mutex> guard(files_mutex_);
        file_streams_[0] << log.prefix << " " << log.time << " " << log.msg
                         << std::endl;
      }
    }
  }
