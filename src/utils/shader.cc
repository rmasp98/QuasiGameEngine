...
GLuint LoadShaders(std::vector<std::string> file_paths) {
  ...
  for (GLuint i_shader = 0; i_shader < file_paths.size(); i_shader++) {
    ...
    if ((file_paths[i_shader].compare(file_paths[i_shader].size() - 3, 3,
      ...
      std::cerr << "Shaders do not have the extension: .vs, .gs, or .fs; or "
                   "two of the same shader is loaded"
                << std::endl;
      return 0;
    }
  ...
  if (vert_index && fragment_index) {
    ...
    std::cerr << "You have not loaded at least a vertex and fragment shader"
              << std::endl;
    return 0;
  }
}
